	...
	vector<double>&                  cgauss)
{
	...
	/*           求每个面的边界         */
	/************************************************************************/
	vector<vector<int>> facesaround( input_faces.size(), vector<int>(0));
	int k ;
	for (int i = 0; i <  input_faces.size(); i++)
	{

		for (int j =0; j < input_faces.size(); j++)
		{	
			k = 0;
			for (int ii = 0; ii < 3; ii++)
			{
				...
				{
					if(input_faces[i][ii]==input_faces[j][iii])   ++k;
				}
			}	
			if(k ==2) facesaround[i].push_back(j);
		}
	}
	...
	double dc, dx, dy,dz;
	int parts = faces/(numk);      //姣忎竴涓鍩熺偣鐨勪釜鏁?
	parts = parts*2;
	double dist;
...
//--------------------------------------------------------------------------
			...
			{
				int A = facesaround[landmarks[n]][num];
				double AA=cgauss[input_faces[A][0]]+cgauss[input_faces[A][1]]+cgauss[input_faces[A][2]];
				double BB=cgauss[input_faces[landmarks[n]][0]]+cgauss[input_faces[landmarks[n]][1]]+cgauss[input_faces[landmarks[n]][2]];
				if(AA < BB)  {landmarks[n] =A;}
			}
//-----------------------------------------------------------------------------------------------------------------------------------
			for (unsigned int i=0; i < vertIndex.size(); i++)
			{
                ...
                double aa = pow( e, -1/(dnormal*dnormal+0.01));
				dist=dnormal+sqrt((dx*dx+dy*dy+dz*dz)*numk /faces);//*numk /faces
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
   ...
   retrieve_data( prhs[2], facenormal, npoints, ndims );
	...
	PerformSupermeshSLIC( input_verts,input_faces,facenormal,landmarks, klabels,cgauss);
  
    // return the indexes
    plhs[0] = mxCreateDoubleMatrix( klabels.size(), 1, mxREAL);
    plhs[1] = mxCreateDoubleMatrix(landmarks.size(), 1, mxREAL);  
    double* indexes = mxGetPr(plhs[0]);
	for (int i = 0; i < klabels.size(); i++)
	{
		indexes[i] = klabels[i];
	}
    double* landmark = mxGetPr(plhs[1]);
	for (int i = 0; i < landmarks.size(); i++)
	{
		landmark[i] = landmarks[i];
	}
