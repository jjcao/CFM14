function [X,F,out]= OptStiefelGBBM(X,M,fun, opts, varargin)
%-------------------------------------------------------------------------
...
GX = G'*X;  %用于求梯度 G-M*X*G'*X

if invH
    GXT = G*X'*M;  H = 0.5*(GXT - GXT');  RX = H*M*X;  
    % GXT 用于W=G*X'*M-M*X*G'  RX用于 (I-0.5*tau*W*M)X
else
    if opts.projG == 1
        U =  [G, X];    V = [X, -G];       VU = V'*U;
    elseif opts.projG == 2
...
dtX = G - M*X*GX;     nrmG  = norm(dtX, 'fro');
...
for itr = 1 : opts.mxitr
    ...
    while 1
        % calculate G, F,
        if invH
            [X, infX] = linsolve(eye(n) + tau*H*M, XP - tau*RX);
        else
...
out.feasi = norm(X'*M*X-eye(k),'fro');
if  out.feasi > 1e-13
    X = MGramSchmidt_m(X,M);
    [F,G] = feval(fun, X, varargin{:});
    out.nfe = out.nfe + 1;
    out.feasi = norm(X'*M*X-eye(k),'fro');
end
